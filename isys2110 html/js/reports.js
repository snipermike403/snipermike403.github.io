...
function initializeReportsPage() {
    ...
    const generateReportBtn = document.querySelector('#reports-content .generate-btn');
    if (generateReportBtn) {
        generateReportBtn.addEventListener('click', showGenerateReportForm);
    }
    
    const exportBtn = document.querySelector('#reports-content .export-btn');
    if (exportBtn) {
        exportBtn.addEventListener('click', exportReports);
    }
    
    const searchBtn = document.querySelector('#reports-content .search-btn');
    if (searchBtn) {
        searchBtn.addEventListener('click', applyReportFilters);
    }
    
    // Add event listeners for view buttons in the reports table
    const viewButtons = document.querySelectorAll('#reports-content .view-btn');
    viewButtons.forEach(button => {
        button.addEventListener('click', function() {
            const row = this.closest('tr');
            const taskName = row.cells[1].textContent;
            showReportDetails(taskName);
        });
    });
    
    // Add event listeners for export buttons in the reports table
    const rowExportButtons = document.querySelectorAll('#reports-content .reports-table .export-btn');
    rowExportButtons.forEach(button => {
        button.addEventListener('click', function() {
            const row = this.closest('tr');
            const taskName = row.cells[1].textContent;
            exportSingleReport(taskName);
        });
    });
}
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="generate-report-form">
                    ...
                    <div class="form-group">
                        <label for="include-tasks">Include Tasks:</label>
                        <select id="include-tasks" multiple>
                            <option value="task1">Target Empire Ads</option>
                            <option value="task2">Patrol X Zone</option>
                            <option value="task3">Reconnaissance Mission</option>
                        </select>
...
function showReportDetails(taskName) {
    ...
    const reportData = {
        ...
        taskReport: 'Task completed successfully. All objectives were met within the allocated time frame. No resistance encountered.',
            ...
            <div class="modal-body">
                <div class="report-summary">
                    <div class="summary-row">
                        ...
                        <span class="summary-value">${reportData.taskName}</span>
                    </div>
                    <div class="summary-row">
            ...
            <div class="modal-body">
                <form id="export-form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="checkbox-group">
                            <div class="checkbox-option">
                                <input type="checkbox" id="include-task-report" name="sections" value="task-report" checked>
                                <label for="include-task-report">Task Report</label>
                            </div>
                            <div class="checkbox-option">
...
function exportSingleReport(taskName) {
    // Create modal for export options
    const modal = document.createElement('div');
    modal.className = 'modal';
    modal.innerHTML = `
    ...
    form.addEventListener('submit', (e) => {
        ...
        document.body.removeChild(modal);
        // Show success message
        showNotification(`Report "${taskName}" exported successfully!`, 'success');
    });
}
